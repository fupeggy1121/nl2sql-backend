...
class SessionMemory:
    ...
    def is_followup_query(self, current_input: str) -> bool:
        ...
        # 长查询也可能是追问: 包含明确的修改/增删指令词（高置信度子串）
        _strong_followup = [
            '不做', '去掉', '去除', '取消', '删掉',
            '加上', '加个', '增加', '添加',
            '并展示', '并显示', '并增加', '并添加',
    ...
    def _resolve_reference(self, current_input: str, last_ctx: Dict) -> str:
        ...
        try:
            ...
            prompt += (
                ...
                f"使其不依赖上下文也能被正确理解。\n"
                f"要求:\n"
                f"1. 保留上一轮查询的所有条件（站点名/设备名/批次号/状态过滤等），除非用户明确要求去除\n"
                f"2. 保留上一轮 SQL 中的多表关联逻辑（如通过载具组关联站点），用自然语言描述出来\n"
                f"3. 只替换用户追问中明确要求修改的部分（如去掉数量限制、增减字段列、改条件等）\n"
                f"4. 如用户说'不做数量限制'/'不限制'，改写后不得包含任何数量限制描述\n"
                f"5. 如用户说'展示/增加XX字段'，在改写中明确包含该字段名\n"
                f"6. 只输出改写后的查询语句，不要解释\n"
                f"7. 使用中文自然语言，不要输出 SQL"
            )
