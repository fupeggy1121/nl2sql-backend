...
9. **禁止把"当前"误解为时间过滤**：查询中出现"当前"、"现在"等词语时，若其修饰的是实体状态（如"当前可用的载具"、"当前在制品"），**禁止**据此添加任何 `gmt_create`、`gmt_update` 等创建/更新时间的范围过滤条件。"当前可用载具"的正确过滤只有 `status = 2`（可用）和 `sub_status = 0`（空载），不涉及时间字段。只有当用户明确提及时间范围（如"最近1天"、"昨天"、"本周"等）时，才添加时间过滤。
10. **禁止自行添加 LIMIT**：除非用户查询中明确提到了"前N条"、"Top N"、"限制N条"等数量限制词，否则禁止在 SQL 中添加 LIMIT 子句。如果用户查询上下文中包含"(限制 N 条)"提示，才添加 LIMIT N。
