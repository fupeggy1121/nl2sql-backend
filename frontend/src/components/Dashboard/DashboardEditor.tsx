...
export function DashboardEditor({ existing, onSave, onClose }: Props) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 400, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.3)' }}>
      <div style={{ background: '#fff', borderRadius: 10, boxShadow: '0 8px 32px rgba(0,0,0,0.18)', padding: '28px 28px 24px', width: '90vw', maxWidth: 520, maxHeight: '90vh', overflowY: 'auto' }}>
        {/* Header */}
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 22 }}>
          <span style={{ fontSize: 16, fontWeight: 700, color: '#111827' }}>{existing ? '编辑看板' : '新建看板'}</span>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14, marginBottom: 22 }}>
          <label style={{ fontSize: 12, color: '#6b7280' }}>
            ...
            <input value={name} onChange={e => { setName(e.target.value); setError('') }}
              ...
              style={{ display: 'block', width: '100%', marginTop: 5, padding: '8px 10px', border: '1px solid #d1d5db', borderRadius: 6, fontSize: 13, boxSizing: 'border-box' }} />
          </label>
          <label style={{ fontSize: 12, color: '#6b7280' }}>
            描述（可选）
            <textarea value={description} onChange={e => setDescription(e.target.value)}
              ...
              style={{ display: 'block', width: '100%', marginTop: 5, padding: '8px 10px', border: '1px solid #d1d5db', borderRadius: 6, fontSize: 13, resize: 'vertical', fontFamily: 'inherit', boxSizing: 'border-box' }} />
          </label>
        </div>
        ...
        <div style={{ marginBottom: 20 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
            <span style={{ fontSize: 13, fontWeight: 600, color: '#374151' }}>全局过滤条件</span>
            <button onClick={addFilter}
              style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '4px 10px', background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: 5, fontSize: 12, color: '#1d4ed8', cursor: 'pointer' }}>
              <Plus size={11} /> 新增过滤器
            </button>
          </div>
          ...
          {filters.map((f, idx) => (
            <div key={f.id} style={{ border: '1px solid #e5e7eb', borderRadius: 7, padding: 12, marginBottom: 10, background: '#fafafa' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                <label style={{ fontSize: 12, color: '#6b7280' }}>
                  ...
                  <input value={f.label} onChange={e => updateFilter(f.id, { label: e.target.value })}
                    ...
                    style={{ display: 'block', width: '100%', marginTop: 3, padding: '5px 8px', border: '1px solid #d1d5db', borderRadius: 5, fontSize: 12, boxSizing: 'border-box' }} />
                </label>
                <label style={{ fontSize: 12, color: '#6b7280' }}>
                  字段列名
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginTop: 8 }}>
                <label style={{ fontSize: 12, color: '#6b7280' }}>
                  ...
                  <select value={f.type} onChange={e => updateFilter(f.id, { type: e.target.value as FilterType })}
                    style={{ display: 'block', width: '100%', marginTop: 3, padding: '5px 8px', border: '1px solid #d1d5db', borderRadius: 5, fontSize: 12 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
          <button onClick={onClose}
            style={{ padding: '8px 18px', background: '#fff', border: '1px solid #d1d5db', borderRadius: 6, fontSize: 13, cursor: 'pointer', color: '#374151' }}>
            取消
          </button>
