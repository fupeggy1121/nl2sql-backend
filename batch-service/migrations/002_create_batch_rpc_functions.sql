-- ============================================================
-- 批次作业 RPC 函数
-- 使用 PL/pgSQL 实现原子性事务，确保多表操作的一致性
-- ============================================================

-- ─────────────────────────────────────────────────────────────
-- 1. 出站确认 RPC
--    对应前端 handleConfirmOutstation 的完整逻辑
-- ─────────────────────────────────────────────────────────────
...
DECLARE
  ...
  v_result          jsonb;
BEGIN
  -- 1. 读取并锁定批次记录（batches.id 是 uuid 类型，需要显式转换）
...
DECLARE
  ...
  v_result          jsonb;
BEGIN
  -- 1. 读取并锁定批次（batches.id 是 uuid 类型）
  SELECT * INTO v_batch FROM batches WHERE id = p_batch_id::uuid FOR UPDATE;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Batch not found: %', p_batch_id;
  END IF;
  ...
  -- 5. 更新子批次（sub_batches.id 是 uuid，需要将 text[] 转为 uuid[]）
  UPDATE sub_batches SET
    status     = '加工中',
    updated_at = now()
...
DECLARE
  ...
  v_result        jsonb;
BEGIN
  -- 1. 读取并锁定目标批次（batches.id 是 uuid 类型）
